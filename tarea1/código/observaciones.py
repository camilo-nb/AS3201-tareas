...
for j in range(3):
    ...
    for i, n in enumerate([2, 4, 5, 6, 8]):
        ...
        plt.text(0.3, 0.85, r"$l^{II}=$"+"{:.2f}".format(lii[5*j+i]), fontsize=5, ha='center', va='center', transform=axs[n-1].transAxes)
        plt.text(0.3, 0.75, r"$b^{II}=$"+"{:.2f}".format(bii[5*j+i]), fontsize=5, ha='center', va='center', transform=axs[n-1].transAxes)
